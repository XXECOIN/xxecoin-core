...
#if defined(NDEBUG)
# error "XXEcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfdf81242984a8ac79a29426c4ab2c8cfd55abb6c1973e9c1845e6bdd0922c067");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // XXEcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "XXEcoin Signed Message:\n";
...
{
    // XXEcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 15 * 60; // XXEcoin: 15 minutes
static const int64 nTargetSpacing = 7.5 * 60; // XXEcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // XXEcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // XXEcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd37900252cb11416363b7654df9e2057ed0777f2b8da05c88793a20a86aa0b09");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Rafa Nadal slams fans following ATP Cup defeat by Novak Djokovic";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04207fb1880605df911d21eab07a6c297437f7f02cc34c19db1e07a0ab6ee9220c8ae698178f3bfbdb4dcc49f6781df12285e4b13bab3b9d2732d41ce48acaa022") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1581413877;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084626194;

        if (fTestNet)
        {
            block.nTime    = 1581413853;
            block.nNonce   = 385400336;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb7a9c6df2b0fa30cb21a006a72c9159c6b77a6eefc9d5f4fc0a7c523736396d3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc2, 0xb4, 0xde }; // XXEcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XXEcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XXEcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static XXEcoinMiner(CWallet *pwallet)
{
    printf("XXEcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xxecoin-miner");
    ...
    try { loop {
        ...
        printf("Running XXEcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("XXEcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&XXEcoinMiner, pwallet));
}
